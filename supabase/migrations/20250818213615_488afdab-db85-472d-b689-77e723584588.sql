...
BEGIN
    ...
    WHERE g.status = 'waiting'
        AND g.current_participants < g.max_participants  -- CORRECTION: Permettre de rejoindre dès qu'il y a de la place
        AND g.current_participants >= 1  -- CORRECTION: Au moins 1 participant (pas de minimum bloquant)
        AND g.created_at > NOW() - INTERVAL '3 hours'  -- Groupes récents seulement
        AND public.calculate_distance(user_latitude, user_longitude, g.latitude, g.longitude) <= search_radius
        AND (g.is_scheduled = false OR g.is_scheduled IS NULL)  -- Exclure les groupes planifiés
    ORDER BY g.created_at DESC  -- Les plus récents d'abord
    LIMIT 5;
END;
$$;
...
RAISE NOTICE 'Corrections Supabase terminées: groupes de test créés, logique métier corrigée, données nettoyées';