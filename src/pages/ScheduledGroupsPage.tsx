...
export default function ScheduledGroupsPage() {
  ...
  const fetchScheduledGroups = async () => {
    if (!user) return;
    ...
    try {
      ...
      toast({
        title: "Erreur",
        description: "Impossible de charger vos groupes planifiés",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
  ...
  const handleCancelGroup = async (groupId: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        await fetchScheduledGroups(); // Refresh the list
      } else {
        toast({
          title: "Erreur",
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-3xl font-bold mb-8">Mes groupes planifiés</h1>
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <h1 className="text-3xl font-bold">Mes groupes planifiés</h1>
          <ScheduleGroupButton onScheduled={fetchScheduledGroups} />
        </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                    ...
                    {group.bar_address && (
                      <div className="flex items-center gap-2 sm:col-span-2">
                        <MapPin className="h-4 w-4 text-muted-foreground" />
                        <span className="truncate">{group.bar_address}</span>
                      </div>
                    )}
                  </div>
