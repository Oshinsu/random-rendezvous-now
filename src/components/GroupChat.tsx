...
const GroupChat = ({ groupId, isGroupComplete, barName }: GroupChatProps) => {
  ...
  console.log('🔍 [GroupChat] Rendu avec messages:', messages);
  console.log('🔍 [GroupChat] Nombre de messages à afficher:', messages.length);
      ...
      <CardContent className="space-y-4">
        ...
        <div className="h-64 overflow-y-auto border rounded-lg p-4 bg-gray-50 space-y-3">
          {loading && messages.length === 0 ? (
            ...
            <>
              {console.log('🔍 [GroupChat] Rendu de', messages.length, 'messages')}
              {messages.map((message, index) => {
                const sender = getMessageSender(message);
                const isOwnMessage = message.user_id === user?.id && !message.is_system;
                
                console.log(`🔍 [GroupChat] Rendu message ${index + 1}:`, {
                  id: message.id,
                  sender,
                  isOwnMessage,
