      ...
      <div className="container mx-auto px-4 sm:px-6 text-center relative z-10 max-w-4xl">
        ...
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-display font-extrabold mb-5 sm:mb-6 animate-slide-in-up">
          {ctaTitle.split('prochaine aventure').map((part, index, array) => (
            index < array.length - 1 ? (
              <span key={index}>{part}<span className="font-signature text-3xl sm:text-4xl md:text-5xl bg-gradient-to-r from-brand-400 via-brand-300 to-brand-400 bg-[length:200%_auto] bg-clip-text text-transparent animate-gradient-text block sm:inline">prochaine aventure</span></span>
            ) : (
              <span key={index}>{part}</span>
            )
          ))}
        </h2>
        <p className="text-base sm:text-lg text-neutral-600 mb-8 sm:mb-10 max-w-3xl mx-auto leading-relaxed animate-slide-in-up" style={{ animationDelay: '0.1s' }}>
        ...
        <div className="animate-slide-in-up" style={{ animationDelay: '0.2s' }}>
          <Button 
            ...
            size="lg" 
            className="bg-gradient-to-r from-brand-500 to-brand-600 hover:from-brand-600 hover:to-brand-700 text-white font-bold px-12 sm:px-16 py-6 sm:py-7 text-lg sm:text-xl transform hover:-translate-y-0.5 transition-all duration-300 shadow-glow-strong animate-pulse-glow group w-full sm:w-auto max-w-md sm:max-w-none rounded-2xl"
          >
